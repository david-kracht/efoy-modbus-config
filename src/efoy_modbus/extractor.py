...
_TEXT_REG_RE = re.compile(
    ...
    r'(?:\s+enum)?'             # optional "enum" qualifier
    r'(?:\s+([^\s\d,]+))?'     # 3: optional unit (single non-digit, non-space token)
    r'\s+(\d{1,6})'             # 4: address (Modbus range 1-65535)
    r'\s+(.+)$'                 # 5: description (rest of line)
)
...
def _parse_text_entry(line: str) -> RawRegisterEntry | None:
    ...
    if not m:
        return None
    name, raw_type, unit, raw_address, description = m.groups()
    ...
    return RawRegisterEntry(
        ...
        is_enum=False,  # enum metadata not available from plain text
    )
...
def extract_registers_raw(pdf_path: Path) -> list[RawRegisterEntry]:
    ...
    for page_text in page_texts:
        for line in page_text.splitlines():
            entry = _parse_text_entry(line)
            if entry is None:
                continue
            addr = _addr_to_dec(entry["raw_address"])
            if addr >= 0 and addr not in seen_addrs:
