...
def raw_entry_to_register(
    ...
    # appears on a continuation row that the table parser didn't capture — so always
    # attempt resolve_enum_ref when definitions are available.
    enum_values: dict[int, str] | None = None
    if entry["is_enum"]:
        enum_values = parse_enum_values(entry["description"])
    if enum_values is None and definitions:
